...
img::EasyImage Triangles3D::zBuffer(const ini::Configuration &configuration) {
    ...
    for (auto &curFig : figures) {
        for (auto &curFace : curFig.faces) {
            img::Color lights(lround(curFig.drawColor.red*255), lround(curFig.drawColor.green*255), lround(curFig.drawColor.blue*255));
            image.draw_zbuf_triag(buffer, curFig.points[curFace.pointIndexes[0]], curFig.points[curFace.pointIndexes[1]],
                                  curFig.points[curFace.pointIndexes[2]], d, dx, dy, lights);
        }
    }
